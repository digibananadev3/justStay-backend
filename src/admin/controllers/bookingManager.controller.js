...
const normalizeBooking = (b, prop = null) => {
  ...
  return {
    ...
    identificationProof: booking.identificationProof || {
      type: "",
      number: "",
      documentUrl: "",
    },
    ...
    coGuestDetails: Array.isArray(booking.coGuestDetails)
      ? booking.coGuestDetails
      : [],
...
export const getBookingsOverview = async (req, res) => {
  try {
    ...
    const [todayCheckIns, todayCheckOuts, pendingActions, activeBookings] =
      await Promise.all([
        ...
        RoomBooking.countDocuments({
          $or: [
            { paymentStatus: "pending" },
            {
              ...
              "refund.status": { $in: ["requested", "approved"] },
            },
            { dispute: { $exists: true, $ne: null }, "dispute.status": "open" },
          ],
        }),
    ...
    return res
      ...
      .json({
        success: true,
        data: { todayCheckIns, todayCheckOuts, pendingActions, activeBookings },
      });
  } catch (error) {
...
export const listBookings = async (req, res) => {
  try {
    ...
    if (search) {
      ...
      or.push({ "guestDetails.email": { $regex: search, $options: "i" } });
    }
...
export const listHourlyBookings = async (req, res) => {
  return listByBaseFilter(
    req,
    res,
    { isHourly: true },
    "Error listing hourly bookings",
  );
};
      ...
      data: [
        {
          ...
          refund: {
            ...
            amount: 15115,
            reason: "",
            processedAt: "2025-10-10T05:00:00.000Z",
          },
          dispute: {
            status: "none",
            reason: "",
...
export const listDisputeBookings = async (req, res) => {
  return listByBaseFilter(
    ...
    "Error listing dispute bookings",
  );
};
...
export const listOtaBookings = async (req, res) => {
  return listByBaseFilter(
    req,
    res,
    { source: { $ne: "JustStay App" } },
    "Error listing OTA bookings",
  );
};
...
export const getBookingById = async (req, res) => {
  try {
    ...
    const property = booking.propertyId
      ? {
          name: booking.propertyId.basicPropertyDetails?.name || "",
          city: booking.propertyId.location?.city || "",
        }
      : {};
    return res
      .status(200)
      .json({ success: true, data: normalizeBooking(booking, property) });
  } catch (error) {
...
export const checkInBooking = async (req, res) => {
  try {
    ...
    const updated = await RoomBooking.findByIdAndUpdate(
      id,
      { $set: { status: "CheckIn", actualCheckInAt: new Date() } },
      { new: true },
    );
    if (!updated)
      return res
    ...
    return res
      .status(200)
      .json({ success: true, data: normalizeBooking(updated.toObject()) });
  } catch (error) {
...
export const checkOutBooking = async (req, res) => {
  try {
    ...
    const updated = await RoomBooking.findByIdAndUpdate(
      id,
      { $set: { status: "CheckOut", actualCheckOutAt: new Date() } },
      { new: true },
    );
    if (!updated)
      return res
...
export const updateRefund = async (req, res) => {
  try {
    ...
    const set = {};
    if (status !== undefined) set["refund.status"] = status;
...
export const updateDispute = async (req, res) => {
  try {
    ...
    const set = {};
    if (status !== undefined) set["dispute.status"] = status;
    ...
    if (status === "resolved" || status === "rejected")
      set["dispute.resolvedAt"] = new Date();
    const updated = await RoomBooking.findByIdAndUpdate(
      id,
      { $set: set },
      { new: true },
...
export const otaSync = async (req, res) => {
  try {
    ...
    return res
      ...
      .json({
        success: true,
        message: "OTA sync triggered (stub)",
        data: normalizeBooking(booking?.toObject?.() || booking),
      });
  } catch (error) {
...
export const resendBookingConfirmation = async (req, res) => {
  try {
    ...
    return res
      ...
      .json({
        success: true,
        message: "Confirmation resent (stub)",
        data: normalizeBooking(updated.toObject()),
      });
  } catch (error) {
...
export const contactGuest = async (req, res) => {
  try {
    ...
    return res
      ...
      .json({
        success: true,
        message: `Contacted via ${channel} (stub)`,
        data: normalizeBooking(booking.toObject()),
      });
  } catch (error) {
