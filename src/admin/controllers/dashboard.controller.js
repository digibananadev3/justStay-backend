...
export const getAdminOverview = async (req, res) => {
  try {
    ...
    const [totalBookings, totalRevenueDoc, activeUsers, pendingVerifications] =
      await Promise.all([
        ...
        PropertyInfo.countDocuments({
          "verification.status": { $in: ["Pending", "InReview"] },
        }),
      ]);
    ...
    // Quick actions
    const [withdrawalRequests, failedBookings, newHotelListings] =
      await Promise.all([
        Settlement.countDocuments({
          status: { $in: ["pending", "processing"] },
        }),
        RoomBooking.countDocuments({
          status: { $in: ["Failed", "Cancel", "Cancelled"] },
        }),
        PropertyInfo.countDocuments({
          createdAt: {
            $gte: startOfDayUTC(
    ...
    const [recentBookings, recentSettlements, recentProperties] =
      await Promise.all([
        RoomBooking.find({})
          .sort({ createdAt: -1 })
          .limit(3)
          .select("_id status priceSummary createdAt"),
        Settlement.find({})
          .sort({ createdAt: -1 })
          .limit(2)
        ...
        PropertyInfo.find({})
          .sort({ createdAt: -1 })
          .limit(1)
          .select("_id basicPropertyDetails.name createdAt"),
      ]);
    const recentActivity = [
      ...recentBookings.map((b) => ({
        ...
        amount: b.priceSummary?.totalPaid || 0,
      ...
      ...recentProperties.map((p) => ({
        type: "property",
        id: p._id,
        title: p.basicPropertyDetails?.name,
        status: "New",
    ...
    // KPI tiles
    const hourlyBookings = await RoomBooking.countDocuments({
      bookingType: { $in: ["Hourly", "hourly"] },
    }).catch(() => 0);
    const goLiveHotels = await PropertyInfo.countDocuments({
      ...
      data: {
        ...
        revenueTrends: {
          ...
          averageDaily: Math.round(
            trend.reduce((a, b) => a + b.amount, 0) / trend.length || 0,
          ),
          peakDay: trend.reduce((p, c) => (c.amount > p.amount ? c : p), {
            date: null,
            amount: 0,
          }),
