...
export const getAnalyticsSummary = async (req, res) => {
  try {
    ...
    const overlapMatch = {};
    if (propertyId && mongoose.Types.ObjectId.isValid(propertyId))
      overlapMatch.propertyId = new mongoose.Types.ObjectId(propertyId);
    overlapMatch.$or = [
      {
        "stayDetails.checkInDate": { $lte: end },
        "stayDetails.checkOutDate": { $gte: start },
      },
    ];

    const overlaps = await RoomBooking.find(overlapMatch)
      .select("priceSummary stayDetails source")
      .lean();
    ...
    for (const b of overlaps) {
      ...
      const paid =
        typeof price.totalPaid === "number"
          ? price.totalPaid
          : typeof price.basePrice === "number"
            ? price.basePrice
            : 0;
      totalRevenue += paid;

      const cin = b.stayDetails?.checkInDate
        ? new Date(b.stayDetails.checkInDate)
        : null;
      const cout = b.stayDetails?.checkOutDate
        ? new Date(b.stayDetails.checkOutDate)
        : null;
      if (cin && cout) {
        // overlap nights
        ...
        const nights = Math.max(
          0,
          Math.round(
            (endOfDayUTC(e).getTime() - startOfDayUTC(s).getTime()) /
              msPerNight,
          ),
        );
    ...
    const daysInRange = Math.max(
      1,
      Math.round(
        (endOfDayUTC(end).getTime() - startOfDayUTC(start).getTime()) /
          msPerDay,
      ) + 1,
    );
    ...
    const occupancyRate =
      availableRoomNights > 0
        ? (consumedRoomNights / availableRoomNights) * 100
        : 0;
    ...
    const sources = sourceAgg.map((s) => ({
      name: s._id,
      percent: Math.round((s.count / totalSrc) * 100),
    }));
